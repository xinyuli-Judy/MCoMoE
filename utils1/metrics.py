...
import numpy as np
from sklearn.metrics import accuracy_score, roc_auc_score, precision_recall_curve, auc as sk_auc, f1_score
...
class MLMetricsFixed:
    ...
    def update(self, label, prediction, other=None):
        ...
        if len(label) == 0 or len(prediction) == 0:
            print("Warning: Empty label or prediction array")
            return

        try:

            if len(label) > 0:
                self.acc = accuracy_score(label, np.round(prediction))
            else:
                self.acc = 0


            if len(np.unique(label)) > 1 and len(label) > 1:
                self.auc = roc_auc_score(label, prediction)
            else:
                self.auc = 0.5
            ...
            if len(np.unique(label)) > 1 and len(label) > 1:
                ...
                self.aupr = sk_auc(recall, precision)
            else:
                self.aupr = 0.5

            if len(label) > 0:
                self.f1 = f1_score(label, np.round(prediction))
            else:
                self.f1 = 0
