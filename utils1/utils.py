...
def read_csv(path):
    ...
    structs = []
    for item in df['icshape']:
        try:

            structs.append([float(x) for x in str(item).split(',')])
        except ValueError:
...
#     targets = df[label].to_numpy().astype(np.float32).reshape(-1, 1)
#     return rnac_set,sequences, structs, targets
...
def one_hot_encode(sequences, kmer=3, max_length=None):

    kmer_dict = {}
    alphabet = 'ACGT'


    kmers = [''.join(p) for p in product(alphabet, repeat=kmer)]
    kmer_dict = {kmer: i for i, kmer in enumerate(kmers)}

    embeddings = []
    for seq in sequences:
        seq = seq.upper().replace('U', 'T')
        encoding = np.zeros((len(kmer_dict), max_length))


        for i in range(len(seq) - kmer + 1):
            mer = seq[i:i + kmer]
            if mer in kmer_dict:
                pos = min(i, max_length - 1)
                encoding[kmer_dict[mer], pos] = 1
